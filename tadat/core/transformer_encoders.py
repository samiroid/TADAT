...
def encode_sequences(docs, tokenizer=None, encoder=None, max_input_size=BERT_MAX_INPUT,
                    cls_features=False, batchsize=DEFAULT_BATCH_SIZE, device=None):  
    #BERT
    if not tokenizer:
...
def encode_multi_sequences(docs, max_sequences, tokenizer=None, encoder=None,
                            max_input_size=BERT_MAX_INPUT, cls_features=False, 
                            batchsize=DEFAULT_BATCH_SIZE, tmp_path=None, device=None):  
    tmp_fname = None      
    if tmp_path:        
    ...
    for j in range(n_batches):
        ...
        if len(batch) > 0:
            sys.stdout.write("\nbatch:{}/{} (size: {})".format(j+1,n_batches, str(len(batch))))
            sys.stdout.flush()
            cls_vec, pool_vec = multi_sequence_features(batch, max_sequences, 
                                                        tokenizer, encoder, max_input_size,
                                                        device)                        
            feats = cls_vec if cls_features else pool_vec                                   
...
def multi_sequence_features(docs, max_sequences, tokenizer, encoder, max_input_size, device):    
    docs_tensor = []    
